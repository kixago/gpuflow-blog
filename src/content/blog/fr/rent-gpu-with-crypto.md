...
**Confidentialité et exigences de vérification réduites.** De nombreuses plateformes GPU natives crypto fonctionnent sans vérification KYC (Know Your Customer) obligatoire. Vous pouvez louer des ressources informatiques en utilisant uniquement une adresse de portefeuille et un solde de cryptomonnaie. Cela séduit les utilisateurs soucieux de la confidentialité, les personnes dans des régions avec une infrastructure bancaire limitée et les développeurs qui préfèrent les transactions pseudonymes. Les paiements en cryptomonnaie permettent des flux de travail impossibles avec la banque traditionnelle. Par exemple, vous pouvez [entraîner des modèles LLM sur des ensembles de données sensibles](/fr/private-llm-fine-tuning-guide) sans lier votre identité au fournisseur de calcul.
