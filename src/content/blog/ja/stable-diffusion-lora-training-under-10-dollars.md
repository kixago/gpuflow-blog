...
20～30枚の控えめなデータセットから始めてください。保守的な設定でトレーニングします。より大きなプロジェクトに拡大する前に、結果を徹底的にテストしてください。試行ごとのコストは、反復が実用的になるほど低く、最初の数回のトレーニング実行を本番出力ではなく学習体験として扱います。この同じワークフローは他のモデルタイプにも適用されます。画像ではなくテキストを扱っている場合は、同じ分散型GPUインフラストラクチャを使用した[大規模言語モデルのファインチューニング](/ja/private-llm-fine-tuning-guide)に関するガイドをご覧ください。
