...
20~30개의 이미지로 구성된 적당한 데이터 세트로 시작하세요. 보수적인 설정으로 훈련하세요. 더 큰 프로젝트로 확장하기 전에 결과를 철저히 테스트하세요. 시도당 비용이 충분히 낮아서 반복이 실용적입니다—첫 몇 번의 훈련 실행을 프로덕션 출력이 아닌 학습 경험으로 취급합니다. 이 동일한 워크플로는 다른 모델 유형에도 적용됩니다. 이미지 대신 텍스트로 작업하는 경우 동일한 분산형 GPU 인프라를 사용하여 [대규모 언어 모델 미세 조정](/ko/private-llm-fine-tuning-guide)에 대한 가이드를 참조하세요.
