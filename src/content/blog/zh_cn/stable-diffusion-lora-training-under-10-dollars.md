...
从二十到三十张图像的适度数据集开始。以保守的设置进行训练。在扩展到更大的项目之前彻底测试您的结果。每次尝试的成本足够低，使得迭代是切实可行的—将您的前几次训练运行视为学习经验而非生产输出。这个相同的工作流程适用于其他模型类型。如果您使用文本而不是图像，请参阅我们关于使用相同去中心化GPU基础设施[微调大型语言模型](/zh_cn/private-llm-fine-tuning-guide)的指南。
