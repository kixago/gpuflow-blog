...
```
ar/      de/      en/      es/      fr/      he/      hi/      ja/      ko/      pt_br/   ru/      zh_cn/   zh_tw/
```
...
I need it written like an educated white male in his late 40's wrote it, with absolutely no AI fluff or jargon and no "bro" verbiage. Professional, relatable.. Absolutely no emojis anywhere. This is the english version.. Lets start with Arabic .. keep the same filename, and exactly the same images across the board (the alt text can be changed to the language at hand but not the location and filename .. that remains the same.. .Below is the article I want translated .. All keys remain in english. The translation should be 1:1 BUT it needs to be in the languages native speech. In other words, if a foreign language uses the same saying in a different pattern or different verb, use the native speech; otherwise, no modification or creativity needed.

All frontmatter keys remain in english, but the title, description translates ... filenames images same in english all across the board ... all links are the same but in native lang directory ... The same images will be used across all languages so whatever link you see for images will be ../_images/whatever.png ... Note!!!! Alt text should be translated natively for good SEO!
Below is the english version of my article .. Do not translate the full file at once .. I want you to do it in thirds so you or I are not overwhelmed .. Split article into thirds, translate then move on to the next third, then the 3rd third ..

All communication with me is in English regardless of whatever language you are on ... File names are the same (png's) ... Every single translation has the same file names across each language sitting in their respective directories .. URL structure is gpuflow.app/ but the is blog.gpuflow.app/language/post ... All links will be in the format [some alt text](/en/link-to-post).  Do not add the .md to any links! Make sure "en" is changed to the native language currently on. External links not associated with GPUFlow should be linked to their respective home page regardless of language as i have no control over that .. Remember to follow the frontmatter, that this is an Astro blog, and SEO is most important to get visitors. If you have any questions before you get started, ask ... Never assume anything. Translate everything in alphabetical order starting with ar and so on until zh_tw
